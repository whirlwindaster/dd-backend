...
export class Game {
    ...
    demoPhase() {
        ...
        if (!this.players.has(this.#state.bid.uuid)) {
            logger.warn(`bidder ${this.#state.bid.uuid} not found`);
            this.#sendToAllPlayers({
    ...
    async gameEvent(from_uuid: string, message: GenericMessageToAPI) {
        switch (message.category) {
            ...
            case 'bid': {
                ...
                this.#sendToAllPlayers({
                    ...
                    name: this.players.get(from_uuid)?.name || 'unknown',
                    moves: message.moves,
                    log: `${this.players.get(from_uuid)?.name} bids ${message.moves} moves`,
                });
